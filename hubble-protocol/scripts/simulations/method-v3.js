...
async function main() {
    ...
    const hourlyInterval = 24
    const interval = hourlyInterval * 60 // 4 hours
    ...
    try {
        ...
        for (let i = 0; i < trades.length; i++) {
            ...
            if ((i+1) % interval != 0 && i+1 != trades.length) continue

            // flush daily data
            _depth /= interval // once every interval
            ...
            const apr = (unrealizedPnl - _unrealizedPnl) * 1440 * 36500 / (makerLiqTarget * interval)
            const net_apr = unrealizedPnl * 1440 * 36500 / (makerLiqTarget * (i+1))
            data.push({
                ...
                position,
                openNotional,
                unrealizedPnl: unrealizedPnl - _unrealizedPnl,
            })
            _numTrades = 0
            _volume = 0
            _slippage = 0
        ...
        console.log({ numTrades, volume, avg_slippage: slippage / numTrades, avg_depth: depth / (minutes / interval + 1) })
        const csvWriter = createCsvWriter({
            path: `${__dirname}/${liquidityTarget/1e6}m-${days}-days-${hourlyInterval}h-2.csv`,
            header: [
                {id: 'epoch', title: 'epoch'},
                {id: 'markPrice', title: 'markPrice'},
                {id: 'apr', title: 'apr'},
                {id: 'net_apr', title: 'net_apr'},
